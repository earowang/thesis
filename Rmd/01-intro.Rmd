...
Missing data perhaps creates an obstacle in the analysis, but the invisibles may mask some data gems. To reveal them, a new data structure is designed for indexing missing values in time, and many operations and plots are built around it. Chapter \@ref(ch:mists) develops exploratory and explanatory tools for missing patterns in time, that fits into a tidy workflow.

## Summary

Chapter \@ref(ch:calendar-vis) provides full details of the calendar plot, algorithm and application. Chapter \@ref(ch:tsibble) explains the new data abstraction--tsibble--and illustrates how it can be used to for the basis of exploratory methods, visualisation and modelling of temporal data. Chapter \@ref(ch:mists) outlines brief categories for temporal missing patterns, and provides tools to easily work with indexed missing values in data. 
