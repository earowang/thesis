...
@r4ds proposed the tidy data workflow, to give a conceptual framework for exploring data (as described in Figure \@ref(fig:tsibble-fit)). In the temporal domain, data with time information arrives at the "import" stage. A new abstraction, *tsibble*, introduced in this chapter, is the gatekeeper at the "tidy" stage, to verify if the raw temporal data is appropriate for downstream analytics. The exploration loop will be aided with declarative grammars, yielding more robust and accurate analyses.
...
This chapter is structured as follows. Section \@ref(sec:str) reviews temporal data structures corresponding to time series and longitudinal analysis, and discusses "tidy data". Section \@ref(sec:semantics) proposes contextual semantics for temporal data, built on top of tidy data principles. The concept of data pipelines, with respect to the time domain, is discussed in depth in Section \@ref(sec:pipeline), followed by a discussion of the design choices made in the software implementation in Section \@ref(sec:software). Two case studies are presented in Section \@ref(sec:cases) illustrating temporal data exploration using the new infrastructure. Section \@ref(sec:conclusion) summarises current work and discusses future directions.
...
@wickham2014tidy coined the term "tidy data", to standardize the mapping of the semantics of a dataset to its physical representation. In tidy form, rows correspond to observations and columns to variables. Tidy data is a rephrasing of the second and third normal forms from relational databases (citation to Codd needed?), but the explanation in terms of observations and variables is easier to understand because it uses statistical terminology.
...
The choice of tidy representation of temporal data arises from a data- and model-oriented perspective, which can accommodate all of the operations that are to be performed on the data in time-based contexts. Figure \@ref(fig:tsibble-fit) marks where this new abstraction is placed in the tidy model, which is referred to as a "tsibble". The "tidy data" principles in [ref Tidy Data paper] are extended as *tidy temporal data principles* in tsibble as follows:
...
The authors would like to thank Mitchell O'Hara-Wild for many discussions on the software development and Davis Vaughan for contributing ideas on rolling window functions. We also thank Stuart Lee for the feedback on this manuscript. We are grateful for anonymous reviewers for helpful feedback that has led to many improvements in the paper which forms majority of this chapter. This article was created with knitr [@knitr] and R Markdown [@rmarkdown]. The project's Github repository <https://github.com/earowang/paper-tsibble> houses all materials required to reproduce this article and a history of the changes.
