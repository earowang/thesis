...
```{r tb-sub-html, eval = knitr::is_html_output()}
...
```{r tb-sub, eval = knitr::is_latex_output()}
```

(ref:tb-sub-cap) A small subset of estimates of tuberculosis burden generated by World Health Organization in 2011 and 2012, with 12 observations and 5 variables. The index refers to column `year`, the key to multiple columns: `country` and `gender`, and the measured variable to column `count`.
...
```{r animate, fig.show = "animate", fig.height = 7, fig.cap = "(ref:animate-cap)", interval = 0.5, aniopts = c("autoplay", "controls", "loop"), eval = knitr::is_latex_output()}
```

```{r animate, fig.show = "animate", fig.height = 7, fig.cap = "(ref:animate-cap)", interval = 0.5, ffmpeg.format = "gif", dev = "png", eval = knitr::is_html_output(), fig.process = "animate.gif"}
```
